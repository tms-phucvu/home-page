...
// --- THAY BẰNG ĐOẠN MỚI NÀY ---
    ...
    // Gom tất cả các thành phần phát sáng của lưới, giới hạn tối đa là 1.0 để không bị cháy sáng
    float intensity = clamp(length(color), 0.0, 1.0);

    // Áp dụng độ mờ cạnh (finalFade) và độ mờ prop (opacity) trực tiếp vào cường độ sáng
    intensity *= finalFade * opacity;

    // Vì bạn đang dùng blendFunc(gl.ONE, gl.ONE),
    // Ta chỉ cần xuất màu RGB thuần khiết nhân với cường độ, alpha đặt bằng intensity hoặc 1.0 tùy ý.
    // Cách này loại bỏ hoàn toàn hiện tượng đổ bóng bẩn/viền đen ở rìa nét vẽ.
    gl_FragColor = vec4(t * intensity, intensity);
}`
